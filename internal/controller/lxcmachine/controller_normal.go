...
func (r *LXCMachineReconciler) reconcileNormal(ctx context.Context, cluster *clusterv1.Cluster, lxcCluster *infrav1.LXCCluster, machine *clusterv1.Machine, lxcMachine *infrav1.LXCMachine, lxcClient *lxc.Client) (ctrl.Result, error) {
	...
	if !ptr.Deref(cluster.Status.Initialization.InfrastructureProvisioned, false) {
		...
		conditions.MarkFalse(lxcMachine, infrav1.InstanceProvisionedCondition, infrav1.WaitingForClusterInfrastructureReason, clusterv1beta1.ConditionSeverityInfo, "")
	...
	if lxcMachine.Spec.ProviderID != nil {
		...
		if err != nil {
			if strings.Contains(err.Error(), "Instance not found") {
				...
				conditions.MarkFalse(lxcMachine, infrav1.InstanceProvisionedCondition, infrav1.InstanceDeletedReason, clusterv1beta1.ConditionSeverityError, "Instance %s does not exist anymore", lxcMachine.GetInstanceName())
				return ctrl.Result{}, nil
			}
	...
	if dataSecretName == nil {
		if !util.IsControlPlaneMachine(machine) && !ptr.Deref(cluster.Status.Initialization.ControlPlaneInitialized, false) {
			...
			conditions.MarkFalse(lxcMachine, infrav1.InstanceProvisionedCondition, clusterv1beta1.WaitingForControlPlaneAvailableReason, clusterv1beta1.ConditionSeverityInfo, "")
			return ctrl.Result{}, nil
		}

		log.FromContext(ctx).Info("Waiting for the Bootstrap provider controller to set bootstrap data")
		conditions.MarkFalse(lxcMachine, infrav1.InstanceProvisionedCondition, infrav1.WaitingForBootstrapDataReason, clusterv1beta1.ConditionSeverityInfo, "")
		return ctrl.Result{}, nil
	}
	...
	if err != nil {
		if utils.IsTerminalError(err) {
			...
			conditions.MarkFalse(lxcMachine, infrav1.InstanceProvisionedCondition, infrav1.InstanceProvisioningAbortedReason, clusterv1beta1.ConditionSeverityError, "Failed to create instance spec: %s", err.Error())
			return ctrl.Result{}, nil
		}
		if strings.HasSuffix(err.Error(), "context deadline exceeded") {
			...
			conditions.MarkFalse(lxcMachine, infrav1.InstanceProvisionedCondition, infrav1.CreatingInstanceReason, clusterv1beta1.ConditionSeverityWarning, "Instance creation still in progress: %s", err.Error())
			return ctrl.Result{RequeueAfter: 10 * time.Second}, nil
		}
		conditions.MarkFalse(lxcMachine, infrav1.InstanceProvisionedCondition, infrav1.InstanceProvisioningFailedReason, clusterv1beta1.ConditionSeverityWarning, "Failed to create instance: %s", err.Error())
